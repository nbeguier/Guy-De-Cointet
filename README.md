...
- `crypto_vigenere.py` — chiffrement/déchiffrement par Vigenère.
- `crypto_all_rot.py` — application de ROT-n sur des textes.
- `crypto_anagrammes.py` — générateur d'anagrammes.

## Tests (`tests/`)

- `run.sh` — vérifie que les messages déchiffrés connus sont retrouvés par `search_pattern.py`.
- `fake_book.txt` — livre synthétique utilisé pour tester la détection de ponctuation.
